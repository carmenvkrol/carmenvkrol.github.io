...
7. **Set `noindex` to `false`** in `src/site.ts` — do this last. While it is
   `true`, every page carries `noindex, nofollow`, which keeps unfinished copy
   out of search results. Leaving it on after launch means nobody ever finds the
   site; turning it off before the copy is real means Google indexes
   "Placeholder — offer one" and serves it for months. The work-in-progress
   banner is a separate flag, `showConstructionNotice`.
